            ...
            <?php foreach ($accounts as $username => $account): ?>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        ...
                        <button type="submit" class="btn btn-sm btn-outline-danger" form="<?= h($deleteFormId) ?>" aria-label="Delete user <?= h((string) $username) ?>" onclick="return confirm('Delete this user?');">Delete</button>
                    </td>
                </tr>
                <?php $formIndex++; ?>
            <?php endforeach; ?>
